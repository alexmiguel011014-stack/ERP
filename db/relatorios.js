...
async function getDRE(dataInicio, dataFim) {
	...
	const resumoVendas = await getAsync(
		"SELECT COUNT(*) AS vendas, COALESCE(SUM(total + COALESCE(desconto, 0)), 0) AS receitaBruta, COALESCE(SUM(desconto), 0) AS descontos FROM Vendas WHERE status = 'finalizada' AND COALESCE(origem, '') != 'importacao_financeiro_historico' AND DATE(data_venda) BETWEEN ? AND ?",
		[inicio, fim],
	);
...
async function getRelatorioVendas(dataInicio, dataFim) {
	...
	const porPagamento = await allAsync(
		"SELECT COALESCE(forma_pagamento, '---') AS forma_pagamento, COUNT(*) AS vendas, SUM(total + COALESCE(desconto, 0)) AS faturamento, SUM(desconto) AS descontos FROM Vendas WHERE status = 'finalizada' AND DATE(data_venda) BETWEEN ? AND ? GROUP BY forma_pagamento ORDER BY faturamento DESC",
		[inicio, fim],
	);
	const devolucoesLinha = await getAsync(
	...
	const historicoSemCMVLinha = await getAsync(
		"SELECT COALESCE(SUM(total), 0) AS valor FROM Vendas WHERE status = 'finalizada' AND origem = 'importacao_financeiro_historico' AND DATE(data_venda) BETWEEN ? AND ?",
		[inicio, fim],
	);
