(function () {
  ...
  function carregarEstoqueNegativo() {
    if (!window.api || !window.api.getEstoqueNegativo) {
      listaEstoque.innerHTML = '<div class="empty-state">API indisponvel.</div>';
      return;
    }
    ...
    window.api
      ...
      .then(function (dados) {
        ...
        dados.forEach(function (item) {
          ...
          top.innerHTML =
            '<span class="venda-id">' + item.produto_nome + "</span>" +
            '<span class="venda-total" style="color: #DC2626;">-' + Math.abs(item.quantidade_estoque) + " un.</span>";
      ...
      .catch(function (err) {
        listaEstoque.innerHTML =
          '<div class="empty-state">Erro ao carregar estoque: ' + err + "</div>";
      });
  }
