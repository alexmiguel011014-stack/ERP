(function () {
  ...
  var filtroCategoriasProduto = document.getElementById("filtroCategoriasProduto");
  ...
  var btnLimparFiltrosProduto = document.getElementById("btnLimparFiltrosProduto");
  ...
  function abrirModalAddCat() {
    ...
    if (window.erpBanco && window.erpBanco.categorias && window.erpBanco.categorias.proximoCodigo) {
      window.erpBanco.categorias.proximoCodigo().then(function (codigo) {
  ...
  btnSalvarAddCat.addEventListener("click", function () {
    ...
    if (!window.erpBanco || !window.erpBanco.categorias || !window.erpBanco.categorias.salvar) {
      mostrarMensagem("API indisponível.", "erro");
      return;
    }
    btnSalvarAddCat.disabled = true;
    window.erpBanco.categorias.salvar(nome, paiId).then(function (resultado) {
      fecharModalAddCat();
  ...
  function atualizarSkuNovo() {
    ...
    if (!window.erpBanco || !window.erpBanco.produtos || !window.erpBanco.produtos.proximoSku) { skuInput.value = ""; return; }
    window.erpBanco.produtos.proximoSku().then(function (sku) {
      skuInput.value = sku;
    }).catch(function () {
      skuInput.value = "";
    });
  ...
  function popularFiltroCategoriasProduto() {
    ...
    var raizes = todasCategorias.filter(function (c) { return !c.categoria_pai_id; });
    if (raizes.length === 0) {
      filtroCategoriasProduto.innerHTML = '<div class="empty-state" style="padding:8px 0; font-size:0.78rem;">Nenhuma categoria.</div>';
      return;
    }
    filtroCategoriasProduto.innerHTML = raizes.map(function (c) {
      var id = String(c.id);
      var marcado = categoriasFiltroSelecionadas.indexOf(id) !== -1 ? "checked" : "";
      return '<label class="filtro-check"><input type="checkbox" data-cat-filtro="' + id + '" ' + marcado + '> ' + esc(c.nome) + '</label>';
    }).join("");
    Array.prototype.forEach.call(filtroCategoriasProduto.querySelectorAll("[data-cat-filtro]"), function (chk) {
      chk.addEventListener("change", function () {
        ...
        else if (!chk.checked && pos !== -1) categoriasFiltroSelecionadas.splice(pos, 1);
        renderizarProdutos();
      });
    });
  }
  ...
  function carregarProdutos() {
    if (!window.erpBanco || !window.erpBanco.produtos || !window.erpBanco.produtos.detalhados) {
  ...
  function produtoNasCategoriasFiltro(p) {
    ...
    return categoriasFiltroSelecionadas.some(function (id) { return idsProduto.indexOf(id) !== -1; });
  }

  function produtoNoFiltroEstoque(p) {
    var variacoes = p.variacoes || [];
    if (filtroSemEstoque && filtroSemEstoque.checked) {
      if (!variacoes.some(function (v) { return Number(v.quantidade_estoque) <= 0; })) return false;
    }
    if (filtroEstoqueBaixo && filtroEstoqueBaixo.checked) {
      if (!variacoes.some(function (v) {
  ...
  function renderizarProdutos() {
    ...
    var produtosFiltrados = produtosCarregados.filter(function (p) {
      if (filtro && [p.nome, tagsCategorias(p).join(" ")].join(" ").toLowerCase().indexOf(filtro) === -1) return false;
    ...
    if (produtosFiltrados.length === 0) {
      tbodyProdutos.innerHTML = '<tr><td colspan="6"><div class="empty-state">' +
        (produtosCarregados.length === 0 ? "Nenhum produto cadastrado ainda." : "Nenhum produto encontrado para os filtros aplicados.") +
        '</div></td></tr>';
      return;
    }
    produtosFiltrados.forEach(function (p) {
      ...
      var skus = (p.variacoes || []).map(function (v) { return v.sku; }).filter(Boolean);
      tdCodigo.innerHTML = '<span class="prod-codigo">' + esc(skus.join(", ") || "---") + '</span>';
  ...
  form.addEventListener("submit", function (e) {
    ...
    if (!window.erpBanco || !window.erpBanco.produtos || !window.erpBanco.produtos.salvar || !window.erpBanco.produtos.atualizar) {
    ...
    var operacao;
    try {
      operacao = editandoId ? window.erpBanco.produtos.atualizar(editandoId, dados) : window.erpBanco.produtos.salvar(dados);
    } catch (erroSync) {
      mostrarMensagem("Erro ao salvar: " + (erroSync && erroSync.message ? erroSync.message : erroSync), "erro");
      btnSalvar.disabled = false; btnSalvar.textContent = "Salvar Produto"; salvando = false;
      return;
    }
    operacao.then(function () {
      mostrarMensagem(editandoId ? "Produto atualizado com sucesso!" : "Produto salvo com sucesso!", "sucesso");
      form.reset(); limparEdicao(); erpCategoryStore.refresh(); carregarProdutos(); atualizarSkuNovo();
  ...
  btnLimpar.addEventListener("click", function () {
    ...
    mensagem.className = "mensagem"; mensagem.style.display = "none"; atualizarSkuNovo();
  });

  btnCancelarEdicao.addEventListener("click", function () {
    limparEdicao(); form.reset(); erpCategoryStore.refresh();
    mensagem.className = "mensagem"; mensagem.style.display = "none"; atualizarSkuNovo(); nomeInput.focus();
  });
  ...
  if (filtroEstoqueBaixo) filtroEstoqueBaixo.addEventListener("change", renderizarProdutos);
  if (filtroSemEstoque) filtroSemEstoque.addEventListener("change", renderizarProdutos);
