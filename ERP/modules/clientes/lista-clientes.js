(function () {
  ...
  function carregarClientes() {
    if (!window.api || !window.api.getClientes) {
      tbody.innerHTML = '<tr><td colspan="6"><div class="empty-state">API indisponível.</div></td></tr>';
      return;
    }
    window.api.getClientes().then(function (lista) {
      ...
      tbody.innerHTML = '<tr><td colspan="6"><div class="empty-state">Erro ao carregar.</div></td></tr>';
    });
  }
  ...
  function renderizarTabela() {
    ...
    var filtrados = clientes.filter(function (c) {
      if (!q) return true;
      var txt = [c.nome, c.cpf_cnpj, c.telefone, c.email || ""].join(" ").toLowerCase();
      return txt.indexOf(q) !== -1;
    });

    totalClientes.textContent = filtrados.length + " cliente" + (filtrados.length !== 1 ? "s" : "");
    if (filtrados.length === 0) {
      tbody.innerHTML = '<tr><td colspan="6"><div class="empty-state">Nenhum cliente encontrado.</div></td></tr>';
      return;
    }
    ...
    filtrados.forEach(function (c) {
      ...
      tdCod.innerHTML = '<span class="cli-codigo">' + esc(c.codigo || ("C" + String(c.id).padStart(4, "0"))) + '</span>';
  ...
  function abrirMovimentacoes(c) {
    ...
    window.api.movimentacoesCliente(c.id).then(function (dados) {
