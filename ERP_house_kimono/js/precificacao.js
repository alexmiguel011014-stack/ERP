(function () {
  ...
  function dadosFiltrados() {
    ...
    return pricingData.filter(function (p) {
      ...
      if (!q) return true;
      return (fmtCodigo(p.produto_id) + " " + p.produto_nome).toLowerCase().indexOf(q) !== -1;
    });
  }
  ...
  function renderizar() {
    ...
    filtrados.forEach(function (p) {
      ...
      var precoCalculado = calcPrecoVenda(p.preco_custo, p.impostos_extras, margemReal);
      ...
      tdCod.innerHTML = '<span class="prod-codigo">' + esc(fmtCodigo(p.produto_id)) + '</span>';
