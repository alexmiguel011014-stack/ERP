...
contextBridge.exposeInMainWorld("api", {
  ...
  registrarEntradaEstoque: (dados) => ipcRenderer.invoke("registrar-entrada-estoque", dados),
  getMovimentacoesEstoque: (limite) => ipcRenderer.invoke("get-movimentacoes-estoque", limite),
  getEstoqueBaixo: () => ipcRenderer.invoke("get-estoque-baixo"),
  salvarEstoqueMinimo: (variacaoId, valor) => ipcRenderer.invoke("salvar-estoque-minimo", variacaoId, valor),
  converterOrcamento: (vendaId) => ipcRenderer.invoke("converter-orcamento", vendaId),
  getFornecedores: () => ipcRenderer.invoke("get-fornecedores"),
  salvarFornecedor: (dados) => ipcRenderer.invoke("salvar-fornecedor", dados),
  atualizarFornecedor: (id, dados) => ipcRenderer.invoke("atualizar-fornecedor", id, dados),
  removerFornecedor: (id) => ipcRenderer.invoke("remover-fornecedor", id),
  criarPedidoCompra: (dados) => ipcRenderer.invoke("criar-pedido-compra", dados),
  getPedidosCompra: () => ipcRenderer.invoke("get-pedidos-compra"),
  getItensPedidoCompra: (pedidoId) => ipcRenderer.invoke("get-itens-pedido-compra", pedidoId),
  receberPedidoCompra: (pedidoId) => ipcRenderer.invoke("receber-pedido-compra", pedidoId),
  cancelarPedidoCompra: (pedidoId) => ipcRenderer.invoke("cancelar-pedido-compra", pedidoId),
  ...
  getFluxoCaixa: (inicio, fim) => ipcRenderer.invoke("get-fluxo-caixa", inicio, fim),
  getRelatorioVendas: (inicio, fim) => ipcRenderer.invoke("get-relatorio-vendas", inicio, fim),
  getCurvaABC: (inicio, fim) => ipcRenderer.invoke("get-curva-abc", inicio, fim),
  unlockWithProfile: (senha) => ipcRenderer.invoke("unlock-with-profile", senha),
  setVendedorKey: (senha) => ipcRenderer.invoke("set-vendedor-key", senha),
  removeVendedorKey: () => ipcRenderer.invoke("remove-vendedor-key"),
  hasVendedorKey: () => ipcRenderer.invoke("has-vendedor-key"),
});
