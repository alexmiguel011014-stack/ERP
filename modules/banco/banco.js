(function () {
		...
		window.erpBanco.banco
			...
			.then(function (res) {
				if (res && res.ok) {
					...
					mostrarMensagem(
						"Senha incorreta ou usuário sem perfil admin.",
						"erro",
					);
					senhaAdmin.value = "";
					senhaAdmin.focus();
	...
	function carregarResumo() {
		resumoTabelas.innerHTML = window.erpSkeletonCards
			? window.erpSkeletonCards(6)
		...
		window.erpBanco.banco
			...
			.then(function (linhas) {
				...
				if (!linhas || linhas.length === 0) {
					resumoTabelas.innerHTML =
						'<div class="empty-state">Nenhuma tabela encontrada.</div>';
					return;
				}
				linhas.forEach(function (l) {
					...
					card.type = "button";
					card.className =
						"resumo-card" + (selectTabela.value === l.tabela ? " ativo" : "");
					card.innerHTML =
						'<div class="nome">' +
						esc(l.tabela) +
						"</div>" +
			...
			.catch(function (erro) {
				resumoTabelas.innerHTML =
					'<div class="empty-state">Erro ao carregar resumo: ' +
					esc(erro) +
					"</div>";
			});
	}
		...
		listarTabelas().then(function (tabelas) {
			...
			if (atual && tabelas.indexOf(atual) !== -1) {
				...
				containerTabela.innerHTML =
					'<div class="empty-state">Selecione uma tabela acima para visualizar os registros.</div>';
	...
	function consultarTabela(tabela) {
		containerTabela.innerHTML =
			'<div class="empty-state">Consultando ' + esc(tabela) + "...</div>";
		infoTabela.textContent = "";
		Array.prototype.forEach.call(
			resumoTabelas.querySelectorAll(".resumo-card"),
			function (c) {
				c.classList.toggle(
					"ativo",
					c.querySelector(".nome").textContent === tabela,
				);
			},
		);
		window.erpBanco.banco
			...
			.then(function (res) {
				infoTabela.innerHTML =
					...
					"</strong> &middot; " +
					res.total +
					" registros (exibindo até " +
					res.limite +
					")";
				if (!res.colunas || !res.colunas.length || !res.linhas.length) {
					containerTabela.innerHTML =
						'<div class="empty-state">Tabela vazia.</div>';
					return;
				}
			...
			.catch(function (erro) {
				containerTabela.innerHTML =
					'<div class="mensagem error" style="display:block;">' +
					esc(erro || "Falha ao consultar tabela.") +
					"</div>";
			});
	}
