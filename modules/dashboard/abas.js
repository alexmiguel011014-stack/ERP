(function () {
	...
	function montarBarra(buscaEl) {
		...
		tabInicio.innerHTML =
			...
			"<span>Início</span>";
		tabInicio.addEventListener("click", function () {
			focar("inicio");
		});
	...
	function abrirIframe(id, titulo, src, icone) {
		if (!barra) {
			requestAnimationFrame(function () {
				abrirIframe(id, titulo, src, icone);
			});
			return;
		}
		if (abas[id]) {
		...
		var loader = window.erpCriarLoader
			? window.erpCriarLoader(loaderWrap, { tamanho: 64 })
			: null;
	...
	function abrirNativa(id, titulo, painelEl, icone) {
		if (!barra) {
			requestAnimationFrame(function () {
				abrirNativa(id, titulo, painelEl, icone);
			});
			return;
		}
		if (!abas[id]) {
	...
	// Mapa arquivo -> {id, titulo, arquivo} dos itens do sidebar que devem
	// abrir como aba em vez de navegar a janela inteira. Antes era um objeto
	// fixo aqui mesmo; agora é derivado de window.erpModulosCarregados (ver
	// docs/MODULE_MANIFEST.md), que navbar.js popula antes de montar a
	// sidebar. Calculado sob demanda dentro do listener de clique (não no
	// topo do script) porque abas.js carrega e roda de forma síncrona, mas a
	// busca de módulos de navbar.js é assíncrona (IPC) — no momento do clique
	// em si a sidebar já está renderizada, então os dados já chegaram.
	function mapaModulosAba() {
		var mapa = {};
		var modulos = window.erpModulosCarregados || [];
		modulos.forEach(function (m) {
			if (m.tipo === "pagina" && m.navbar && m.navbar.abaDashboard) {
				mapa[m.entrada] = {
					id: m.id,
					titulo: m.navbar.label,
					arquivo: "../" + m.id + "/" + m.entrada,
				};
			}
		});
		return mapa;
	...
	document.addEventListener("click", function (e) {
		...
		var modulo = mapaModulosAba()[arquivo];
