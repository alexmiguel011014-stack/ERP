...
const CHAVE = require("crypto")
	.createHash("sha256")
	.update("erp_housekimono:" + SENHA)
	.digest("hex");
				...
				conn.serialize(() => {
					...
					SCHEMA_LEGADO.forEach((sql) => {
						conn.run(sql, (e3) => {
							if (e3 && !erro) erro = e3;
							if (--pendente === 0)
								conn.close(() => (erro ? reject(erro) : resolve()));
						});
					});
				});
			});
...
(async () => {
	...
	for (const t of [
		...
		check(
			"tabela nova criada: " + t,
			tabelas.indexOf('"' + t + '"') !== -1 || tabelas.indexOf(t) !== -1,
			tabelas,
		);
	}
	...
	check(
		"produto legado presente",
		produtos.length === 1 && produtos[0].nome === "Quimono Trançado",
		JSON.stringify(produtos),
	);
	check(
		"variação legada presente",
		produtos[0] && produtos[0].variacoes.length === 1,
		JSON.stringify(produtos[0] && produtos[0].variacoes),
	);
	const cats = await db.getListCategoriasWithUsage();
	check(
		"categorias legadas presentes",
		cats.length === 2,
		JSON.stringify(cats),
	);
	const clientes = await db.getClientes();
	check(
		"cliente legado presente",
		clientes.length === 1,
	...
	check(
		"busca cliente maiúscula com acento",
		cliAcento.length === 1,
		JSON.stringify(cliAcento),
	);
	...
	const novo = await db.salvarProduto(
		{
			nome: "Faixa Preta",
			categoriasSelecionadas: [novaCat.id],
			variacoes: [
				{
					sku,
					preco: 120,
	...
	check(
		"agora há 2 produtos",
		depois.length === 2,
		JSON.stringify(depois.map((p) => p.nome)),
	);
